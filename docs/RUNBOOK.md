...
- [x] Vaultwarden is live at `https://vault.intern.example.com` and `rbw list` works from the Mac (see `apps/internal/vaultwarden/README.md` in infra-gitops).
...
- [x] Headscale: `headscale users list` shows `your-user`. Mint a key with
      `./scripts/mac/new-laptop-key.sh`, which writes it into vault item `headscale-preauth`.
      **Keys last 24h** — re-run the script if the install slips to another day.
      Note `headscale preauthkeys list` redacts keys (`hskey-auth-...***`, all entries an
      identical 27 characters), so a listed value never matches the full one in the vault.
      That is redaction, not a truncated or wrong key; check `acl_tags`, `reusable` and
      `expiration` to confirm a key rather than comparing the string.
...
- [x] Repo pushed: `github.com/your-github-user/omarchy-setup` (private). The raw URL needs a token — it 404s without one — so the bootstrap command in §3 passes `Authorization: Bearer`.
- [x] USB stick with the current Omarchy ISO — written with balenaEtcher (which verifies its own write). `./scripts/mac/latest-iso.sh --download` reads
...
- [ ] Username `your-user`, strong password (this becomes the LUKS passphrase too).
...
```bash
curl -fsSL https://raw.githubusercontent.com/your-github-user/omarchy-setup/main/bootstrap.sh | bash
```
...
- [ ] `~/.local/share/omarchy-setup/scripts/verify.sh` → all green. Typical first-run reds: kubectl (kubeconfig item missing). Home Assistant is reached at `http://homeassistant.ts.example.com:8123` — the MagicDNS name, because mDNS does not cross the mesh.
...
- [ ] `proj` opens the fuzzy switcher; `clone your-github-user/<repo>` works.
...
- [ ] Rotate `github-token-laptops`, Headscale pre-auth keys (they expire anyway), Home Assistant and AFFiNE tokens → update vault items → `chezmoi apply` + `agents-setup.sh` on each laptop.
      `github-token-laptops` currently holds a **classic** PAT whose scopes include `admin:org`, `admin:enterprise` and `delete_repo`, and classic PATs inherit org access, so it reaches your-org repos too. Replacing it with a fine-grained token (owner `your-github-user`, Contents RW / Workflows RW / Metadata R / Pull requests RW) is the single biggest reduction in blast radius for a lost laptop. Until then, revoking it is step one of the lost-laptop drill below.
...
| `affine-mcp` | Login | Claude MCP | password = `aff_mcp_v1.` token, custom field `url` = `https://notes.intern.example.com/api/workspaces/<workspace-id>/mcp` |
...
Create the AFFiNE credential **inside the self-hosted instance** (`https://notes.intern.example.com` → Settings → Integrations → MCP Server), not through the button on affine.pro, which sends you to AFFiNE Cloud. A cloud-issued token fails against the self-hosted endpoint with a 401 that is byte-identical to the one you get for sending no credential at all, so there is nothing in the error to tell you which mistake you made. Transport is streamable HTTP; the endpoint is stateless and returns no `mcp-session-id`.

`rbw` reads items but cannot create them or set custom fields. Creating and updating these items is done on the Mac with the official Bitwarden CLI (`brew install bitwarden-cli`, `bw config server https://vault.intern.example.com`). The laptops only ever read, so they only need `rbw`.
