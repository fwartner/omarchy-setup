...
Personal workstation fleet on Omarchy Linux, separate from the Pixel & Process Mac, wired into the existing infrastructure (Headscale mesh, `*.intern.example.com` services, Vaultwarden, the shared Kubernetes cluster) and built for agentic coding.
...
| Backups | restic → Hetzner Object Storage (`nbg1`), own bucket and own key pair, nightly systemd timer, `~/Projects` + `~/.config` excluded caches | Laptops are disposable only if backups are boring. Separate bucket and credentials from `example-cluster-backups` so a stolen laptop key is not also a key to customer database backups. |
...
- **Deploy Vaultwarden to the shared cluster first** — it lives in `infra-gitops` as `apps/internal/vaultwarden` (branch `feat/vaultwarden`), hostname `vault.intern.example.com`. Nothing else in the bootstrap works without it. Create your account, disable signups, test with `rbw` from the Mac, put the SQLite data dir into the cluster's PVC backup.
...
- Headscale: user `your-user` already exists. `tag:laptop` is forced server-side from the pre-auth key and needs no ACL policy — headscale runs `policy.mode: database` with no rows, i.e. allow-all inside the tailnet. Mint keys with `scripts/mac/new-laptop-key.sh` right before each install (they expire in 24h); it stores them in Vaultwarden.
...
- Push this repo to `github.com/your-github-user/omarchy-setup` (private).
...
- Install Omarchy (full disk, LUKS, keyboard `de`, user `your-user`, hostname per inventory).
...
- Vault availability: Vaultwarden does not exist yet; it goes onto the shared cluster as step one (`infra-gitops/apps/internal/vaultwarden`). Bootstrap joins Headscale first (pre-auth key is the only pasted secret) and only then talks to `secrets.intern`, so the vault can stay mesh-only.
