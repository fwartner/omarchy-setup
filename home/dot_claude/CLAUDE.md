...
- Kubernetes access from these machines is read-only (`kubectl get/describe/logs`). Deploys happen via `pixelandprocess-gitops` PRs, never `kubectl apply` from a laptop.
